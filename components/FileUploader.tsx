    ...
    <div className='flex flex-col gap-4 items-center max-w-7xl mx-auto'>
      ...
      {uploadInProgress && (
        <div className='mt-32 flex flex-col justify-center items-center gap-5'>
          ...
          <p className='text-indigo-600 animate-pulse'>{status ? status.toString() : ''}</p>
        </div>
      )}
      <div {...getRootProps()}
      className={`p-10 border border-dashed mt-10 w-[90%] border-indigo-600 text-indigo-600 rounded-lg h-96 flex items-center justify-center ${isFocused || isDragAccept ? "bg-indigo-300" : "bg-indigo-100"}`}
      >
        <input {...getInputProps()} />
